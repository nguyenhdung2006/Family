...
import { Bell, BookOpen, CalendarHeart, ChefHat, Heart, Image, MessageCircle, Sprout } from "lucide-react";
...
const quickLinks = [
  { href: "/family-tree", label: "Family Tree", icon: Sprout, tone: "sage" },
  { href: "/timeline", label: "Memories", icon: BookOpen, tone: "yellow" },
  { href: "/albums", label: "Albums", icon: Image, tone: "blue" },
  { href: "/messenger", label: "Messages", icon: MessageCircle, tone: "wood" },
  { href: "/memorial", label: "Memorial", icon: Heart, tone: "cream" },
  { href: "/kitchen", label: "Kitchen", icon: ChefHat, tone: "sage" }
] as const;

export function HomeDashboard() {
  const { data: user } = useCurrentUser();
  const { data: posts = [] } = useTimelinePosts({ page: 0, size: 3 });
  const { data: members = [] } = useFamilyMembers({ page: 0, size: 8 });
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_22rem]">
            ...
            <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
              <div className="max-w-2xl">
                ...
                <h2 className="mt-4 text-4xl font-black leading-tight text-ink sm:text-5xl">
                  Welcome back, {user?.name?.split(" ")[0] ?? "family"}.
                </h2>
              ...
              <div className="grid min-h-52 min-w-52 place-items-center rounded-lg border border-border-warm bg-white/70 p-5">
                <div className="text-center">
                  <div className="mx-auto grid h-24 w-24 place-items-center rounded-full bg-warm-yellow/25 text-5xl">🏡</div>
          ...
          <Card>
            <CardContent>
              <div className="mb-4 flex items-center justify-between">
                ...
                <Link href="/timeline" className="text-base font-black text-wood">View all</Link>
              </div>
      ...
      <aside className="grid content-start gap-5">
        <Card>
          <CardContent>
            <h3 className="flex items-center gap-2 text-xl font-black text-ink">
              <CalendarHeart className="h-5 w-5 text-wood" /> Upcoming birthdays
            </h3>
            <div className="mt-4 grid gap-3">
              {birthdays.length ? birthdays.map((member) => (
        ...
        <Card>
          <CardContent>
            <h3 className="flex items-center gap-2 text-xl font-black text-ink">
              <Bell className="h-5 w-5 text-wood" /> Recent messages
            </h3>
            <div className="mt-4 grid gap-3">
              ...
              {!rooms.length ? <p className="font-semibold text-muted">No message rooms yet.</p> : null}
            </div>
          </CardContent>
        </Card>
      </aside>
